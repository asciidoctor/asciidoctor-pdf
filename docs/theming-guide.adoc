...
Where the transparent keyword is not recognized, you can use the CMYK array value [0, 0, 0, 0] to get the same effect.
...
In some cases, this can also be expressed using a CMYK array value:
...
In addition to the generic caption category, each of these keys can be set on the caption key nested inside the following block categories: blockquote, code, example, footnotes, image, listing, and table.
