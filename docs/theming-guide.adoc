...
If the filename begins with `./`, it is resolved relative to the current theme file.
Otherwise, the filename is resolved relative to the value of the `pdf-stylesdir` attribute.

The files references by the extends key are loaded in order just like the main theme file.
The keys in each theme, including the main theme, are overlaid onto the keys from the previous theme.
Currently, the base theme is always loaded first.
