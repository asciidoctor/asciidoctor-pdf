...
These margins and used when the document attribute `media` has the value `prepress`.
...
Asciidoctor PDF supports the following features to accommodate publishing:

* Double-sided (mirror) page margins
* Facing pages

These features are activated when you set the `media` attribute to `prepress` in the header of your AsciiDoc document, the CLI, or the API.
The following sections describe the behaviors this setting enables.

=== Double-Sided Page Margins

The page margins for the recto (right-hand, odd-numbered) and verso (left-hand, even-numbered) pages are automatically calculated by replacing the side margins in the value of the `page_margin` key with the value of the `page_margin_inner` and `page_margin_outer` keys.
...
The page margins for the recto and verso pages will be as follows:
...
For documents that use the article doctype, Asciidoctor PDF incorrectly places the document title and table of contents on their own pages, respectively.
This can result in the page numbering and the page facing to be out of sync.
As a workaround, Asciidoctor PDF inserts a blank page, if necessary, to ensure the following pages are recto-facing pages:
