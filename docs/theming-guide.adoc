...
If the filename begins with `./`, it's resolved as is relative to the current theme file.
Otherwise, the filename is resolved in the same way as the value of the `pdf-style` attribute (resolved relative to the value of the `pdf-stylesdir` attribute).
