...
TIP: While the keys can be nested whereever an underscore appears in the name, this nested structure is flatted when the theme is loaded.
...
^[2]^ Controls whether the `page-number` attribute is accessible to the running header and footer content specified in the theme file.
