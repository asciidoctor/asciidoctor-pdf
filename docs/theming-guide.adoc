...
|cover {vbar} title {vbar} toc {vbar} body {vbar} Integer +
(default: body)
|page:
...
. The `cover` value is only recognized if the documet has a front cover page (i.e., `front-cover-image`).
...
|section:
  indent: [0.5in, 0]
|===
. Applies to the section body only, excluding section titles and discrete headings.
