...
In either case, the image is resolved relative to the value of the `pdf-stylesdir` attribute, as previously described.
...
//deprecated
//3+|[#key-prefix-table-row]*Key Prefix:* <<key-prefix-table-row,table-<parity>-row>>^[1]^
//
//|background-color
//|<<colors,Color>> +
//(default: $table-background-color)
...
Otherwise, `-theme.yml` is appended to the name to make the file name (i.e., `<name>-theme.yml`) and is resolved relative to `pdf-stylesdir`, if specified, otherwise the built-in themes dir.
