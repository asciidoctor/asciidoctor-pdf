...
TIP: If you want to specify the location of custom fonts using the `pdf-fontsdir` attribute, yet still be able to reference the bundled fonts, you need to refer to the bundled fonts using the `GEM_FONTS_DIR` token.
...
. The value may be an image file or a PDF file.
...
. The `title` and `toc` values are only recognized if the title page is enabled (i.e., doctype is book or `title-page` attribute is set)
...
. If border-left-width is non-zero, the border is only applied to the left side.
Otherwise, if border-width is non-zero, the border is drawn around the whole block.
. The verse block does not use a fixed-width font by default, which can affect the layout if the content relies on columns.
...
You can also built-in AsciiDoc text replacements like `+(C)+`, numeric character references like `+&#169;+`, hexadecimal character references like `+&#x20ac;+`, and inline formatting (e.g., bold, italic, monospace).
...
. In this case, the target of the image macro is resolved relative to `imagesdir` since it's defined in an AsciiDoc document attribute (unlike in the theme, where it is resolved relative to the value of `pdf-themesdir`).
...
You can change the font color using `rgb` attribute on the `color` element (e.g., `<color rgb="#ff0000">`) and the font family and size using `name` and `size` attributes on the `font` element, respectively (e.g., `<font name="sans" size="12">`).
