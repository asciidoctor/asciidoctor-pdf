...
== Making room for large tables

Large tables may require extra space to layout out.
...
That means we can extend the converter in order to render the table in another document with a custom page size (and orientation), then import those pages back into the main PDF.
...
Some additional work has to be done with attributes to ensure that the scratch document has access to the same context as the main document.
