...
Use to define columns in the running content - up to three sections. Column width is specified using percent symbol which describes how much place between left and right margin will be used. Widths must sum up to 100%. Use `<`, `=` and `>` to define horizontal alignment of content, where `<` is aligning to the left (default behavior), `=` is centering and `>` is aligning to the right.
