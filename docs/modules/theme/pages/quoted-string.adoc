...
Additionally, normal substitutions are applied to the value of content keys for xref:running-content-keys.adoc[running content], so you can use most AsciiDoc inline formatting (e.g., `+*strong*+` or `+{attribute-name}+`) in the values of those keys.
