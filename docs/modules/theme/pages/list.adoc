...
Type can be `disc`, `square`, `circle`, `checked`, `unchecked`, or `biblio` (for the bibliography lists).

When the marker is `biblio`, the `content` key is required.
If the value of the `content` key is empty, it acts like the `unstyled` marker.
