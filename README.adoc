...
* If, for some reason, an inline image cannot be resized to fit within the space allocated to a table cell, a Prawn::Errors::CannotFit error will be thrown (see https://github.com/asciidoctor/asciidoctor-pdf/issues/1608 for workarounds).
This problem should have been solved, but we're still on the lookout for edge cases.
...
If you're in this sitatuion, it might be best to try <<hexapdf>> instead.
...
Start by creating a Ruby file named [.path]_hexapdf-optimizer.rb_, then populate it with the following code:
...
To activate your custom optimizer, load this file when invoking the `asciidoctor-pdf` using the `-r` flag and set the `optimize` attribute as well using the `-a` flag.
 ...
 $ asciidoctor-pdf -r ./hexapdf-optimizer.rb -a optimize basic-example.adoc
