...
All that's needed is Ruby 1.9.3 or better (2.2.1 recommended) and a few Ruby Gems, which we explain how to install in the next section.
...
If you're using {uri-rvm}[RVM], we recommend creating a new gemset to work with Asciidoctor and {project-name}:
 ...
 $ rvm use 2.2.1@asciidoctor-pdf --create

We like RVM because it keeps the dependencies required by various projects isolated.

== Getting started

You can get {project-name} by <<installing-the-published-gem,installing the published gem>> or <<development,running the code from source>>.
...
// Uncomment next line to change page size (default is Letter)
//:pdf-page-size: A4
...
You're also encouraged to try converting this <<README#,README>> as well as the documents in the link:{relfileprefix}examples/[examples directory] to see more of what {project-name} can do.
...
TIP: For best results, be sure to use `bundle exec` whenever invoking the `./bin/asciidoctor-pdf` script .
