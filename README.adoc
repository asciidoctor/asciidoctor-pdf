...
When writing content, it is sometimes important to include pages with styles impossible to achieve currently with Asciidoc, such as individual pages that have full-bleed color that runs right up to the page edge; or custom, complex layouts that might come from specialized design programs like https://inkscape.org[Inkscape].

For physical prints, these types of markings and stylistic flourishes are useful https://fromplantoprototype.com/blog/2019/08/07/importing-pdf-pages-in-asciidoctor-pdf/[to separate chapters and article sections], as it is possible to see the colored page edges when looking at the https://www.alibris.com/glossary/condition#booka[fore-edge] of a printed object. It's a helpful way to physically and visually index a book or a longer article.
...
Note, however, that this will only import a single page from the PDF at a time (according to enhancement https://github.com/asciidoctor/asciidoctor-pdf/issues/177[#177]).
