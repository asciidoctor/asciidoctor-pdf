...
When writing content, it is sometimes important to include pages with styles impossible to achieve currently with Asciidoc.
For example, individual pages that have full-bleed color that runs right up to the page edge.
Or complex layouts that might come from specialized design programs like https://inkscape.org[Inkscape].

For physical prints, these types of markings and stylistic flourishes are useful https://fromplantoprototype.com/blog/2019/08/07/importing-pdf-pages-in-asciidoctor-pdf/[to separate chapters and article sections].
Using colored page edges makes it is possible to look at the https://www.alibris.com/glossary/condition#booka[fore-edge] of a printed object and distinguish the different parts.
It's a helpful way to physically and visually index a book or a longer article.

To add these types of pages to an Asciidoc file, use the following construct:
...
A page break will automatically be inserted before and after the imported PDF page, so there is no need to add extra https://asciidoctor.org/docs/asciidoc-syntax-quick-reference/#breaks[`<<<` markup] around the `image::[]` statement.

Note also that, at the moment, this construct will import only the first page from the PDF (according to enhancement https://github.com/asciidoctor/asciidoctor-pdf/issues/177[#177]).
