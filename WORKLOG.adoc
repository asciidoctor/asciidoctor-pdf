...
* SIMPLE: use literal cell style for theme-related document attributes section of theming guide
...
* FILE ISSUE: if the image type cannot be resolved from file extension (e.g., when target is a URL), honor format attribute like core
...
* autowidth on table doesn't work for multi-line content (width based on normalized value)
...
* FILE ISSUE: enable/implement key table_font_style
...
* prawn-gmagick (fixes PNG transparency issue, adds gif support); don't fail-fast on gif if gem is detected
...
* allow caption placement for table to be configurable (top vs bottom) (#531)
...
* NOTE: when using `single_line: true` on formatted_text, it's necessary to reapply our padding top/bottom from line metrics
...
* a nested image with a relative path in an SVG is resolved relative to the SVG source file (document in README)
