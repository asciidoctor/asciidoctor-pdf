...
* AsciiDoc table cell leaves padding below last block
* go ahead and add separate padding control to label in admonition block; default to padding on content if not specified
...
* should we defer image dimension assignments to calc_image_dimensions for raster images (similar to resize from Prawn SVG)
* we could support the big and small roles on phrase by mapping to base_font_size_large and base_font_size_small
* print alt text if an error is caught when attempting to embed an image (#693)
  - add layout_alt_text helper method
* restrict inline image to height of page (#700)
* FILE ISSUE: allow height of inline image to be set to line height (perhaps 1em?)
