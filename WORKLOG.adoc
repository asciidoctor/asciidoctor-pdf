...
* fix duplicate inline image rendering by ensuring the spacer does not exceed the available width (#388)
  - indicate we are doing a full-line width image so we can align image to left of fragment instead of center
  - see if we can use character spacing to fine-tune reserved space
* use to_pt method in theme_loader (#677)
