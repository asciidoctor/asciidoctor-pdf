...
* SIMPLE: add note to README that Prawn will dynamically subset any fonts provided
...
* FILE ISSUE: implement text_transform key for table_header_cell (in body)
* implement text transform for running content (#702)
* FILE ISSUE: implement line number offset in source listing (rouge)
 ** shouldn't this be fixed in core at the same time?
