...
.1.5.0.alpha.17
* (#85, PR sent) footnotes as chapter endnotes
* (#891, PR available) FontAwesome 5 upgrade
 ** in what case do we need to lazy load the icon font in the font method?
* (#606, PR available) running content on toc and title pages
* does upgrading prawn-svg fix some issues? need to retest
* (maybe) (#191, PR available) text indent for first line of paragraph
* (maybe) add preamble to toc
* (maybe) test suite
* hexadecimal character refs
 ** should we go ahead and add nbsp too?
...
* (#191) implement first-line indent for paragraphs (seems like conflict w/ our text formatter)
